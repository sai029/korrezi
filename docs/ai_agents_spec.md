...
「スコアが上がる」こと自体はタイトルを直接書き換えない。**スコアは Gemini への入力コンテキストであり、
文言生成そのものは毎回 Gemini の自由生成**という2段構成になっている。

```
① 子どもが記事を読む（recordView → updateInterestModel）
     └─ 滞在秒数 → E(i) → 該当カテゴリの DISA スコアに加算（本章「興味スコアリング」参照）

② 24時間ごと（または初回・feed空時）に personalizeArticles が発火
     └─ interest_profile.current_interests を読み取り時 DISA 減衰補正
     └─ 減衰後スコア > 0.5 のカテゴリを上位5件に絞り、文字列化
          例: "スポーツ: 82.3点, 経済・お金: 61.0点, 科学: 38.5点"

③ news_pool の最新20記事それぞれについて、②の文字列をそのまま
   Gemini プロンプトに埋め込んで呼び出す（personalizeOneArticle）
     └─ プロンプトに「この記事を子どもの興味に合わせて」と指示
     └─ Gemini が記事ごとに 3 つを JSON で返す:
          interest_score（0-100・興味を持つ確率）
          rewritten_title（20字以内）
          rewritten_tagline（30字以内）

④ personalized_feed に保存 → フィードのタイトル/タグライン/興味スコアに反映
     └─ 興味別サムネ再生成（Phase③）は interest_context が上位5カテゴリに
        一致する・未読・共有サムネ生成済みの記事のみが対象
```

**重要な設計上の注意**:
- 記事のカテゴリ自体（`interest_context`）は書き換わらない。変わるのは**同じカテゴリ内での切り口・語り口**
- Gemini は「上位興味リスト」というテキストコンテキストを与えられるだけで、スコアと出力の対応関係は
  決定論的ではない（同じ入力でも `temperature: 0.7` により生成結果は毎回多少揺れる）
- 上位5件の抽出元は `current_interests` の**キー全部**。ここに `TOPIC_CATEGORIES`（12分類）以外の
  値が混入すると、それがそのまま「子どもの興味」として Gemini に渡ってしまう
  （2026-07-10 に判明・修正。次項参照）

### 既知の不具合と修正（2026-07-10）

`source_name` 分離前の旧スキーマ記事を経由して `current_interests` にニュースソース名
（例: `tenki.jp`, `Gizmodo`）が書き込まれるケースがあった。これらのキーには
`last_score_updated_at` が伴わないことが多く、**DISA 減衰が一切適用されないまま
永続的に上位へ居座り**、毎日の `personalizeArticles` 実行で「子どもの上位興味」として
Gemini プロンプトにそのまま渡り続けていた。

修正（`functions/src/index.ts`）:
- `personalizeArticles`: `current_interests` を読む際、`TOPIC_CATEGORIES` に含まれる
  キーのみに限定してから減衰計算・上位5件抽出・Gemini プロンプト生成を行う
- `updateInterestModel`: 記事の `interest_context` が `TOPIC_CATEGORIES` に無い場合は
  スコア書き込みをスキップする（旧スキーマ記事がまだ `news_pool` に残っていた場合の保険）

既存ユーザーの汚染データはワンオフスクリプトで個別に浄化済み（コード資産には含まない）。
...
| 項目 | 現状 | 今後 |
|---|---|---|
| `interest_context` | ✅ 採点ゲート（`scoreArticle`）が固定タクソノミー12分類（科学/宇宙/テクノロジー/自然・環境/動物/スポーツ/食べ物/音楽・アート/経済・お金/国際・世界/文化・歴史/社会・くらし）でトピック分類（2026-07-02） | 既存記事は再取得時に上書き済み |
| 旧ソース名カテゴリの興味スコア汚染 | ✅ `personalizeArticles`/`updateInterestModel` に `TOPIC_CATEGORIES` フィルタを追加し、以後は混入・反映されない（2026-07-10）。~~DISA 減衰で自然消滅する~~ という従来の想定は誤りだった（`last_score_updated_at` が無いキーは減衰計算自体が走らず、永続的に残っていた） | 既存ユーザーの汚染データはワンオフスクリプトで個別浄化。全ユーザーへの一括クリーンアップは未実施 |
