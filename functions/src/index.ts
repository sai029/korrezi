...
async function toChildFriendly(a: GNewsArticle): Promise<ChildFriendly> {
  const prompt = [
    ...
    "子ども向けの3キー（display_title / display_tagline / child_body_with_ruby）は、",
    "小学校で習わない漢字に 〔漢字｜よみ〕 の形式でルビを付ける（例: 〔環境｜かんきょう〕）。",
    "ひらがなだけにはしない。parent_summary は大人向けなのでルビを付けない。",
    '- "display_title": 子ども向けの短いタイトル（表示される文字は20文字以内・記号や煽りは不要・ルビ付き）',
    '- "display_tagline": 興味を引く一言（表示される文字は30文字以内・ルビ付き）',
    '- "child_body_with_ruby": 本文（2〜4文・ルビ付き）。',
    '- "parent_summary": 保護者向けの箇条書き要約。各行を「・」で始め、2〜3項目（ルビなし）。',
    "",
    "記事:",
    articleSource(a),
  ].join("\n");
